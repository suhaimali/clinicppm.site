...
// Safe wrapper for DateTimePicker to avoid native dismiss race causing crashes
class SafeDateTimePicker extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    try {
      const PickerImpl = (DateTimePickerModule && (DateTimePickerModule.default || DateTimePickerModule)) || DateTimePickerModule;
...
// Ensure JSX references to `DateTimePicker` use the safe wrapper
const DateTimePicker = SafeDateTimePicker;

// Helper: generate PDF for a single prescription and open share dialog
const generateRxPdf = async (rx, patient) => {
  try {
    const vitalsToHtml = (vitals) => {
      if (!vitals) return '';
      return Object.entries(vitals).filter(([k,v]) => v).map(([k, v]) => `<div><strong>${k.toUpperCase()}:</strong> ${v}</div>`).join('');
    };
    const medsHtml = (meds) => meds && meds.length ? meds.map(m => `<tr><td>${m.name} ${m.strength || ''}</td><td>${m.dosage || ''}</td><td>${m.frequency || ''}</td><td>${m.duration || ''}</td><td>${m.instructions || '-'}</td></tr>`).join('') : '';
    const proceduresHtml = (procs) => procs && procs.length ? procs.map(p => `<tr><td>${p.name}</td><td>₹${p.cost || ''}</td></tr>`).join('') : '';
    const html = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#222} .hdr{background:#00796b;color:#fff;padding:10px;border-radius:6px} .section{margin-top:12px} table{width:100%;border-collapse:collapse} th,td{border:1px solid #ddd;padding:8px;text-align:left} th{background:#f5f5f5}</style></head><body><div class="hdr"><h2>${DOCTOR_NAME}</h2><div style="font-size:12px">${CLINIC_ADDRESS}</div></div><div class="section"><h3>Prescription</h3><div><strong>Patient:</strong> ${patient?.name || rx.patientName || 'Unknown'}</div><div><strong>Date:</strong> ${new Date(rx.date).toLocaleDateString()}</div><div><strong>Diagnosis:</strong> ${rx.diagnosis || ''}</div></div><div class="section"><h4>Vitals</h4>${vitalsToHtml(rx.vitals)}</div><div class="section"><h4>Medicines</h4><table><thead><tr><th>Medicine</th><th>Dosage</th><th>Frequency</th><th>Duration</th><th>Instructions</th></tr></thead><tbody>${medsHtml(rx.medicines)}</tbody></table></div><div class="section"><h4>Procedures</h4><table><thead><tr><th>Procedure</th><th>Cost</th></tr></thead><tbody>${proceduresHtml(rx.proceduresPerformed)}</tbody></table></div>${rx.notes ? `<div class="section"><h4>Notes</h4><div>${rx.notes}</div></div>` : ''}<div style="margin-top:20px;font-size:12px;color:#777">Generated by ${DOCTOR_NAME} on ${new Date().toLocaleString()}</div></body></html>`;
    const { uri } = await Print.printToFileAsync({ html });
    await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: 'Prescription PDF' });
  } catch (err) {
    Toast.show({ type: 'error', text1: 'PDF Error', text2: err.message || 'Could not generate PDF' });
  }
};

// Safety: ensure DateTimePickerAndroid.dismiss exists on Android to avoid native race errors
try {
  if (Platform.OS === 'android') {
    const dtpModule = require('@react-native-community/datetimepicker');
    if (dtpModule && dtpModule.DateTimePickerAndroid && typeof dtpModule.DateTimePickerAndroid.dismiss !== 'function') {
      dtpModule.DateTimePickerAndroid.dismiss = () => {};
    }
  }
} catch (e) {
  // ignore
...
const INITIAL_APPOINTMENTS = [
  { id: 'a1', time: 'Mon Dec 15 2025 | 09:00 AM', patientName: 'Alice Johnson', type: 'Routine Checkup', reason: 'Headache', status: 'Booked' },
  { id: 'a2', time: 'Mon Dec 15 2025 | 10:30 AM', patientName: 'Charlie Brown', type: 'Follow Up', reason: 'Review blood work', status: 'Booked' },
];
...
export default function App() {
  ...
  const handleSavePrescription = (rx) => {
    ...
    setTimeout(() => {
      ...
      const appointmentToRemove = appointments.find(a => a.patientName === p?.name && a.status === 'Booked');
  ...
  const renderContent = () => {
    switch (currentScreen) {
      ...
      case Screens.ADD_RX: {
        ...
        return <NewPrescriptionForm patient={patientFromSelectedData} inventory={inventory} templates={rxTemplates} onSave={handleSavePrescription} onCancel={() => navigate(Screens.RX_HISTORY, patientFromSelectedData)} initialData={isEdit ? selectedData : null} allPrescriptions={prescriptions} onSaveTemplate={handleSaveTemplate} />;
      }
...
const Dashboard = ({ patients, appointments, inventory, templates, procedures, prescriptions, navigate, openDrawer, onDeleteAppt, onUpdateAppt, setPatients }) => {
  ...
  const EditApptModal = () => {
    ...
    const handleSaveChanges = () => {
      onUpdateAppt(editAppt.id, timeString, notes, vitals);
      if (patientData) {
      ...
      <Modal visible={!!editAppt} transparent={true} animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContentLarge}>
            ...
            {showDatePicker && <SafeDateTimePicker value={date} mode={mode} is24Hour={false} display="default" onChange={handleDateChange} />}
          </View>
        </View>
      </Modal>
    )
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContentLarge}>
          <View style={styles.colorfulHeader}>
            ...
            {rx.isTapering ? (
              <View style={{ backgroundColor: Colors.warning, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, marginRight: 10 }}>
                <Text style={{ color: '#FFF', fontWeight: 'bold' }}>Tapering</Text>
              </View>
            ) : null}
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <TouchableOpacity onPress={() => generateRxPdf(rx, { name: rx.patientName })} style={[styles.iconBtn, { marginRight: 8, backgroundColor: Colors.primary + '20' }]}><FontAwesome5 name="file-pdf" size={16} color="#B00020" /></TouchableOpacity>
              <TouchableOpacity onPress={onClose} style={styles.closeBtnHeader}><Ionicons name="close" size={24} color="#FFF" /></TouchableOpacity>
            </View>
          </View>
          <ScrollView style={{padding: 20}}>
            ...
            {rx.notes ? (<View style={styles.noteBox}><Text style={styles.noteTitle}>Booking Notes</Text><Text style={styles.noteContent}>{rx.notes}</Text></View>) : null}
...
const PatientList = ({ patients, navigate, onDelete, onEdit, onBook }) => {
  ...
  const [apptDateObj, setApptDateObj] = useState(new Date()); // State for date object
  const [showDatePicker, setShowDatePicker] = useState(false); // Toggle picker
  ...
  const onDateChange = (event, selectedDate) => {
    if (event && event.type === 'set' && selectedDate) {
      ...
      if (mode === 'date') {
        ...
        setTimeout(() => setShowDatePicker(false), 60);
        setMode('date');
      }
    } else {
      setTimeout(() => setShowDatePicker(false), 60);
        ...
        renderItem={({ item }) => (
          <View key={item.id} style={styles.card}>
            ...
            <View style={styles.cardActions}>
              <TouchableOpacity onPress={() => setBookModal(item)} style={styles.actionLabelBtn}>
                <FontAwesome5 name="calendar-plus" size={14} color={Colors.primary} />
                <Text style={styles.actionLabelText}>Book</Text>
              </TouchableOpacity>
      ...
      <Modal visible={!!bookModal} transparent={true} animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {showDatePicker && (<SafeDateTimePicker value={apptDateObj} mode={mode} is24Hour={false} display="default" onChange={onDateChange} />)}

            <Input label="Booking Notes" value={apptReason} onChange={setApptReason} style={{marginTop: 15}} />
            
            <TouchableOpacity style={[styles.row, { marginBottom: 20 }]} onPress={() => setIsFollowUp(!isFollowUp)}>
              <FontAwesome5 name={isFollowUp ? "check-square" : "square"} size={20} color={Colors.primary} />
              <Text style={{ marginLeft: 10, color: Colors.text }}>Follow Up?</Text>
            </TouchableOpacity>
            
            <View style={styles.row}>
              <TouchableOpacity style={[styles.btnPrimary, { flex: 1, backgroundColor: Colors.subText, marginRight: 10 }]} onPress={() => setBookModal(null)}>
                <Text style={styles.btnText}>CANCEL</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.btnPrimary, { flex: 1 }]} 
                onPress={() => { 
                  onBook(bookModal, getFormattedTimeString(apptDateObj), apptReason, isFollowUp ? 'Follow Up' : 'Consultation'); 
                  setBookModal(null); 
                }}
              >
  ...
  const PatientDetails = ({ patient, labs, navigate }) => {
    ...
    const handleShareID = async () => {
      try {
        if (!viewShotRef.current) throw new Error('Capture reference not ready');
        const uri = await viewShotRef.current.capture();
        await Sharing.shareAsync(uri, { mimeType: 'image/png', dialogTitle: 'Share ID Card (PNG)' });
      } catch (error) {
    ...
    const handleDownloadID = async () => {
      try {
        ...
        if (status !== 'granted') { Toast.show({ type: 'error', text1: 'Permission denied', text2: 'Allow media permissions to save image.' }); return; }
        const asset = await MediaLibrary.createAssetAsync(uri);
        try { await MediaLibrary.createAlbumAsync('Download', asset, false); } catch (e) { /* album may already exist */ }
        Toast.show({ type: 'success', text1: 'Saved', text2: 'ID card saved to gallery.' });
      } catch (error) {
        Toast.show({ type: 'error', text1: 'Save Error', text2: error.message });
      }
    };

    const handleShareWhatsAppText = async () => {
      try {
        const viewLink = (patient.image && patient.image.startsWith('http')) ? patient.image : '';
        const txt = `Dear ${patient.name},\n\nYour *clinic pathappiriyam* ID Card:\n\n*ID:* ${patient.id}\n*Name:* ${patient.name}\n*Phone:* ${patient.phone}\n\nView: ${viewLink}\n\nContact: +918606344694\nThank you!`;
        const encoded = encodeURIComponent(txt);
        const urlScheme = `whatsapp://send?text=${encoded}`;
        const webUrl = `https://api.whatsapp.com/send?text=${encoded}`;
        try { await Linking.openURL(urlScheme); } catch (e) { await Linking.openURL(webUrl); }
      } catch (error) {
        Toast.show({ type: 'error', text1: 'WhatsApp Error', text2: error.message });
      }
    };
  ...
  return (
    <View style={styles.screenContainer}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        <ViewShot ref={viewShotRef} options={{ format: 'png', quality: 1, result: 'tmpfile' }} style={styles.idCard}>
          {/* Header */}
          <View style={styles.idHeaderContainer}>
            <Text style={styles.idCardTitle}>ID CARD</Text>
            {/* Action Icons: Download / WhatsApp Text / Share */}
            <View style={{ position: 'absolute', right: 0, flexDirection: 'row', gap: 12 }}>
              <TouchableOpacity onPress={handleDownloadID} style={{ marginRight: 12 }}>
                <MaterialIcons name="file-download" size={20} color="#FFFFFF" />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleShareWhatsAppText} style={{ marginRight: 12 }}>
...
const PrescriptionHistoryScreen = ({ patient, prescriptions, navigate, onDeleteRx, onEditRx }) => {
  ...
  const sortedFilteredPrescriptions = [...filteredPrescriptions].sort((a, b) => new Date(b.date) - new Date(a.date));
  const displayPrescriptions = filterMode === 'recent' ? sortedFilteredPrescriptions.slice(0, 10) : sortedFilteredPrescriptions;
  ...
  const onDateChange = (event, selectedDate) => { setTimeout(() => setShowDatePicker(null), 60); if (event && event.type === 'set' && selectedDate) { if (showDatePicker === 'start') setStartDate(selectedDate); else if (showDatePicker === 'end') setEndDate(selectedDate); } };
  const RxTableItem = ({ item }) => (
    <View key={item.id} style={styles.rxTableItem}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <TouchableOpacity onPress={() => generateRxPdf(item, patient)} style={[styles.iconBtn, { marginLeft: 5, backgroundColor: Colors.primary + '20' }]}><FontAwesome5 name="file-pdf" size={14} color="#B00020" /></TouchableOpacity>
...
const AllPrescriptionHistoryScreen = ({ allPrescriptions, patients, navigate, onDeleteRx }) => {
  ...
  const sortedFilteredPrescriptionsAll = [...filteredPrescriptions].sort((a, b) => new Date(b.date) - new Date(a.date));
  const displayPrescriptions = filterMode === 'recent' ? sortedFilteredPrescriptionsAll.slice(0, 20) : sortedFilteredPrescriptionsAll;
  const onDateChange = (event, selectedDate) => { setTimeout(() => setShowDatePicker(null), 60); if (event && event.type === 'set' && selectedDate) { if (showDatePicker === 'start') setStartDate(selectedDate); else if (showDatePicker === 'end') setEndDate(selectedDate); } };
  const RxTableItem = ({ item }) => {
    const patientObj = patients.find(p => p.id === item.patientId) || { name: item.patientName };
    return (
      <View key={item.id} style={styles.rxTableItem}>
        <View style={styles.rxTableCol}>
          <Text style={[styles.rxDate, { fontWeight: 'bold', color: Colors.primary }]}>{item.patientName}</Text>
          <Text style={styles.rxDate}>{item.date}</Text>
          <Text style={styles.rxDiagnosis}>{item.diagnosis}</Text>
          <Text style={{ fontSize: 12, color: Colors.subText }}>{item.medicines.length} Meds</Text>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <TouchableOpacity onPress={() => setViewRx(item)} style={[styles.iconBtn, { backgroundColor: Colors.action + '20' }]}><FontAwesome5 name="eye" size={14} color={Colors.action} /></TouchableOpacity>
          <TouchableOpacity onPress={() => generateRxPdf(item, patientObj)} style={[styles.iconBtn, { marginLeft: 5, backgroundColor: Colors.primary + '20' }]}><FontAwesome5 name="file-pdf" size={14} color="#B00020" /></TouchableOpacity>
          <TouchableOpacity onPress={() => onDeleteRx(item.id)} style={[styles.iconBtn, { marginLeft: 5, backgroundColor: Colors.danger + '20' }]}><FontAwesome5 name="trash" size={14} color={Colors.danger} /></TouchableOpacity>
        </View>
      </View>
    );
  ...
  return (
    <View style={styles.screenContainer}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        {showDatePicker && (<SafeDateTimePicker value={showDatePicker === 'start' ? startDate : endDate} mode="date" display="default" onChange={onDateChange} maximumDate={new Date()} />)}
...
const NewPrescriptionForm = ({ patient, inventory, templates, onSave, onCancel, initialData, allPrescriptions, onSaveTemplate }) => {
  ...
  const [rxData, setRxData] = useState(
    initialData
      ? { ...initialData, vitals: initialData.vitals || patient.vitals || {}, templateId: initialData.templateId || 'template-none', proceduresPerformed: initialData.proceduresPerformed || [] }
      : { patientId: patient.id, diagnosis: '', notes: '', isTapering: false, vitals: patient?.vitals || {}, medicines: [], proceduresPerformed: [], templateId: 'template-none' }
  );
  ...
  const [nextVisit, setNextVisit] = useState(initialData?.nextVisit || null);
  const [showNextVisitPicker, setShowNextVisitPicker] = useState(false);
  const [referral, setReferral] = useState(initialData?.referral || '');

  useEffect(() => {
    if (!medSearch || medSearch.trim() === '') return;
    const match = inventory.find(i => i.name.toLowerCase() === medSearch.toLowerCase() || i.name.toLowerCase().startsWith(medSearch.toLowerCase()));
    if (match) {
      setCurrentMed(prev => ({ ...prev, strength: prev.strength || match.strength, dosage: prev.dosage || match.dosage }));
    }
  }, [medSearch, inventory]);
  ...
  useEffect(() => {
    ...
    if (selectedTemplate && selectedTemplate.id !== 'template-none') {
      setRxData(prev => ({ ...prev, diagnosis: selectedTemplate.diagnosis, medicines: selectedTemplate.medicines.map(m => ({ ...m, id: 'med_' + Date.now() + Math.random() })) }));
  ...
  const handleAddMedicine = () => {
    if (!currentMed.name || !currentMed.strength) { Toast.show({ type: 'error', text1: 'Missing Fields', text2: 'Please enter Medicine Name and Strength.' }); return; }
  ...
  const handleFinalSave = () => {
    if (!rxData.diagnosis || rxData.medicines.length === 0) {
      Toast.show({ type: 'error', text1: 'Incomplete Data', text2: 'Please enter a diagnosis and at least one medicine.' });
      return;
    }
    const selectedTemplate = templates.find(t => t.id === rxData.templateId);
    onSave({ ...rxData, templateName: selectedTemplate?.name || 'Custom', nextVisit: nextVisit, referral: referral });
  };

  const handleSaveAsTemplate = () => {
    const tpl = { id: null, name: `Template - ${rxData.diagnosis || 'Custom'} (${Date.now()})`, diagnosis: rxData.diagnosis, medicines: rxData.medicines };
    if (typeof onSaveTemplate === 'function') {
      onSaveTemplate(tpl);
    } else {
      Toast.show({ type: 'info', text1: 'Template', text2: 'No template handler available.' });
    }
  };

  const handleImportRx = (rxId) => {
    const rx = (allPrescriptions || []).find(x => x.id === rxId);
    if (!rx) { Toast.show({ type: 'error', text1: 'Not found', text2: 'Selected prescription not found.' }); return; }
    setRxData(prev => ({ ...prev, diagnosis: rx.diagnosis || prev.diagnosis, notes: rx.notes || prev.notes, isTapering: rx.isTapering || prev.isTapering, medicines: rx.medicines || prev.medicines, vitals: rx.vitals || prev.vitals }));
    Toast.show({ type: 'success', text1: 'Imported', text2: 'Previous prescription imported.' });
  };
  ...
  return (
    <View style={styles.screenContainer}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        <View style={styles.row}><VitalInputRx label="SpO2" unit="%" kbd="numeric" style={{ marginRight: 10 }} /><VitalInputRx label="BP" unit="mmHg" /></View>
        <View style={styles.row}><VitalInputRx label="HR" unit="bpm" kbd="numeric" style={{ marginRight: 10 }} /><VitalInputRx label="Temp" unit="°F" kbd="numeric" /></View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 8 }}>
          <TouchableOpacity style={[styles.iconBtn, { marginRight: 10 }]} onPress={() => setShowPreview(true)}><FontAwesome5 name="eye" size={16} color={Colors.primary} /></TouchableOpacity>
          <TouchableOpacity style={styles.iconBtn} onPress={handleSaveAsTemplate}><FontAwesome5 name="save" size={16} color={Colors.action} /></TouchableOpacity>
        </View>

        {allPrescriptions ? (
          <View style={{ marginTop: 10 }}>
            <Text style={styles.label}>Import from Previous Rx</Text>
            <View style={styles.pickerContainer}>
              <Picker selectedValue={null} onValueChange={(v) => v && handleImportRx(v)}>
                <Picker.Item key="none" label="-- Select --" value={null} />
                {(allPrescriptions || []).filter(x => x.patientId === patient.id).map(rx => <Picker.Item key={rx.id} label={`${rx.date} - ${rx.diagnosis}`} value={rx.id} />)}
              </Picker>
            </View>
          </View>
        ) : null}
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, marginBottom: 8 }}>
          <Text style={[styles.label, { flex: 1, marginTop: 0 }]}>Tapering?</Text>
          <TouchableOpacity style={[styles.toggleBtn, { backgroundColor: rxData.isTapering ? Colors.success : '#CCC' }]} onPress={() => setRxData({ ...rxData, isTapering: !rxData.isTapering })}>
            <FontAwesome5 name={rxData.isTapering ? 'check' : 'times'} size={14} color="z#FFF" />
          </TouchableOpacity>
        </View>

        <Input label="Booking Notes / Additional Instructions" value={rxData.notes} onChange={(t) => setRxData({ ...rxData, notes: t })} multiline={true} style={{ marginBottom: 20 }} />
        ...
        {rxData.medicines.map((med) => (
          <View key={med.id} style={styles.medCard}>
            ...
            <TouchableOpacity onPress={() => handleRemoveMedicine(med.id)} style={[styles.iconBtn, { backgroundColor: Colors.danger + '20' }]}><FontAwesome5 name="trash-alt" size={14} color={Colors.danger} /></TouchableOpacity>
          </View>
        ))}
        ...
        <View style={[styles.addItemBox, { marginTop: 10 }]}>
          ...
          <TextInput style={[styles.inputBox, { marginBottom: 8 }]} placeholder="Search or type medicine..." value={medSearch} onChangeText={t => { setMedSearch(t); setCurrentMed({ ...currentMed, name: t }); }} />
          {(medSearch.length > 0) && (
            <View style={{ maxHeight: 120, backgroundColor: '#FFF', borderRadius: 8, borderWidth: 1, borderColor: '#EEE', marginBottom: 8 }}>
              {(medicineNames.filter(n => n.toLowerCase().includes(medSearch.toLowerCase())).slice(0, 8)).map(name => (
                <TouchableOpacity key={name} style={{ padding: 10, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' }} onPress={() => { setCurrentMed({ ...currentMed, name }); setMedSearch(name); }}>
                  <Text>{name}</Text>
                </TouchableOpacity>
              ))}
              <TouchableOpacity style={{ padding: 10 }} onPress={() => setCurrentMed({ ...currentMed, name: medSearch })}><Text style={{ color: Colors.action }}>Use "{medSearch}" as custom name</Text></TouchableOpacity>
            </View>
          )}
          <View style={styles.row}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.pickerContainer}><Picker selectedValue={currentMed.dosage} onValueChange={(v) => setCurrentMed({ ...currentMed, dosage: v })} style={{ height: 40 }}>{dosageForms.map(f => <Picker.Item key={f} label={f} value={f} />)}</Picker></View>
            </View>
          </View>
          <View style={styles.row}>
            <View style={{ flex: 1, marginRight: 10 }}>
              <Text style={styles.label}>Frequency</Text>
              <View style={styles.pickerContainer}><Picker selectedValue={currentMed.frequency} onValueChange={(v) => setCurrentMed({ ...currentMed, frequency: v })} style={{ height: 40 }}>{FREQUENCY_OPTIONS.map(f => <Picker.Item key={f} label={f} value={f} />)}</Picker></View>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.label}>Duration</Text>
              <View style={styles.pickerContainer}><Picker selectedValue={currentMed.duration} onValueChange={(v) => setCurrentMed({ ...currentMed, duration: v })} style={{ height: 40 }}>{DURATION_OPTIONS.map(d => <Picker.Item key={d} label={d} value={d} />)}</Picker></View>
            </View>
          </View>
          ...
          <TouchableOpacity style={[styles.btnSmall, { backgroundColor: Colors.action }]} onPress={handleAddMedicine}><Text style={{ color: '#FFF', fontWeight: 'bold' }}>ADD TO PRESCRIPTION</Text></TouchableOpacity>
        </View>
        ...
        {rxData.proceduresPerformed.map((proc) => (
          <View key={proc.id} style={[styles.medCard, { borderLeftColor: Colors.dash6 }]}>
            ...
            <TouchableOpacity onPress={() => handleRemoveProcedure(proc.id)} style={[styles.iconBtn, { backgroundColor: Colors.danger + '20' }]}><FontAwesome5 name="trash-alt" size={14} color={Colors.danger} /></TouchableOpacity>
          </View>
        ))}
        ...
        <View style={[styles.addItemBox, { marginTop: 10 }]}>
          ...
          <TouchableOpacity style={[styles.btnSmall, { backgroundColor: Colors.dash6 }]} onPress={handleAddProcedure}><Text style={{ color: '#FFF', fontWeight: 'bold' }}>ADD PROCEDURE</Text></TouchableOpacity>
        </View>

        <View style={{ height: 10 }} />
        <Text style={[styles.label, { marginTop: 10 }]}>Next Visit (optional)</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          <TouchableOpacity style={[styles.pickerBtn, { flex: 1 }]} onPress={() => setShowNextVisitPicker(true)}><Text style={styles.pickerText}>{nextVisit ? new Date(nextVisit).toLocaleString() : 'Set Next Visit'}</Text><FontAwesome5 name="calendar-alt" size={16} color={Colors.primary} /></TouchableOpacity>
          <View style={{ width: 10 }} />
          <TextInput style={[styles.inputBox, { flex: 1 }]} placeholder="Referral / Referred By" value={referral} onChangeText={setReferral} />
        </View>
        {showNextVisitPicker && (<SafeDateTimePicker value={nextVisit ? new Date(nextVisit) : new Date()} mode="datetime" display="default" onChange={(e, d) => { setShowNextVisitPicker(false); if (e?.type === 'set' && d) setNextVisit(d.toISOString()); }} />)}

        <TouchableOpacity style={styles.btnPrimary} onPress={handleFinalSave}><Text style={styles.btnText}>{isEdit ? 'UPDATE' : 'SAVE'} PRESCRIPTION</Text></TouchableOpacity>

        <Modal visible={showPreview} transparent={true} animationType="slide">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContentLarge}>
              <View style={styles.modalHeaderRow}><View style={styles.modalIconBg}><FontAwesome5 name="eye" size={20} color={Colors.action} /></View><View style={{flex:1, marginLeft:10}}><Text style={styles.modalTitleLeft}>Preview Rx</Text><Text style={styles.modalSubTitle}>Preview before saving</Text></View><TouchableOpacity onPress={() => setShowPreview(false)}><Ionicons name="close" size={24} color={Colors.subText} /></TouchableOpacity></View>
              <ScrollView style={{ padding: 20 }}>
                <Text style={{ fontSize: 18, fontWeight: 'bold' }}>{rxData.diagnosis || 'No Diagnosis'}</Text>
                <Text style={{ color: Colors.subText, marginTop: 8 }}>{rxData.notes}</Text>
                <View style={{ marginTop: 12 }}>
                  {rxData.medicines.map(m => (<View key={m.id} style={{ paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' }}><Text style={{ fontWeight: 'bold' }}>{m.name} {m.strength}</Text><Text style={{ color: Colors.subText }}>{m.dosage} | {m.frequency} | {m.duration}</Text></View>))}
                </View>
              </ScrollView>
              <View style={styles.modalFooter}>
                <TouchableOpacity style={styles.btnOutline} onPress={() => setShowPreview(false)}><Text style={styles.btnOutlineText}>CLOSE</Text></TouchableOpacity>
                <TouchableOpacity style={styles.btnBlue} onPress={() => { setShowPreview(false); generateRxPdf(rxData, patient); }}><FontAwesome5 name="file-pdf" size={14} color="#FFF" style={{ marginRight: 8 }} /><Text style={styles.btnBlueText}>EXPORT PDF</Text></TouchableOpacity>
              </View>
            </View>
          </View>
        </Modal>
...
const AddProcedureForm = ({ initialData, patients, onSave, onCancel }) => {
  ...
  const onDateChange = (event, selectedDate) => { setTimeout(() => setShowDatePicker(false), 60); if (event && event.type === 'set' && selectedDate) { setDate(selectedDate); setForm({ ...form, date: selectedDate.toLocaleDateString('en-CA') }); } };
